...
contract VestingTokenClaim is Ownable, ReentrancyGuard {
    ...
    function claimTokens(
        ...
        require(availableAmount > alreadyClaimed, "No tokens available to claim");
        ...
        uint256 currentWeek = (block.timestamp - vestingStartTime) / WEEK_DURATION;
     ...
     */
    function _calculateAvailableAmount(uint256 totalAmount) internal view returns (uint256) {
     ...
     */
    function _getMessageHash(ClaimData memory claimData) internal pure returns (bytes32) {
    ...
    function getClaimableAmount(
        ...
        uint256 nonce,
        uint256 deadline
    ) external view returns (uint256 claimableNow, uint256 totalClaimed, uint256 totalAvailable) {
        ClaimData memory claimData = ClaimData({
            recipient: user,
            totalAmount: totalAmount,
            nonce: nonce,
